...
void handleRoot()
{
  ...
  if (_setupServer)
  {
    Serial1.printf("Opening config.html\n");
    File file = LittleFS.open("/config.html", "r");
    if(!file){
      Serial1.printf("Failed to open config.html for reading\n");
      return;
    } else Serial1.printf("Success to open config.html for reading\n"); 

  // reading from file
  while(file.available()){
    htmlConfig.concat(file.readString());
  }
  file.close();
...
void handleUpdate()
{
  ...
  Serial1.printf("Opening success.html\n");
  File file = LittleFS.open("/success.html", "r");
  if(!file){
    Serial1.printf("Failed to open success.html for reading\n");
    return;
  } else Serial1.printf("Success to open success.html for reading\n"); 

  // reading from file
  while(file.available()){
    //htmlSuccess.concat((char)file.read());
    htmlSuccess.concat(file.readString());
  }
  file.close();
